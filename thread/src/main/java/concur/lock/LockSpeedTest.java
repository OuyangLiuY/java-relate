 ...
 *  header状态：
 *      1.无锁
 *      25:unused + 31:hash +1:unused + 4:age +  1:biased_lock + 2:concur.lock = 64bit
 *      eg:56 + 0 0000 0 01 ->无锁
 *      2.偏向锁
 *      54:thread& + 2:epoch + 1:unused + 4:age + 1:biased_lock + 2:concur.lock =64bit
 *      eg:56 + 0 0000 1 01 ->偏向锁
 *      3.轻量级锁
 *                 62:prt_to_lock_record                        + 2:concur.lock = 64bit
 *      eg:56 + 001010 00 ->轻量级锁
 *      4.重量级锁
 *               62:prt_to_heavyweight_monitor                  + 2:concur.lock  = 64bit
...
public class LockSpeedTest {
    ...
    static class HeavyLockSpeedAnother{
        ...
        public static void main(String[] args) throws InterruptedException {
            ...
            t2.start();
            // 5465ms
            latch.await(); //等到countDown减为0执行下面代码。
            long end = System.currentTimeMillis();
            System.out.println((end - begin) + "ms");
            System.out.println(aa);
